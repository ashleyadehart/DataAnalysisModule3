...
where order_id in (select order_id from orders where status = 'paid')
group by order_id) as paid_orders;
...
group by date(order_datetime)
having count(*) > 3; 
-- No results were rendered when I ran this code. This tells me that total orders did not exceed 3. 
...
select (100.0 * SUM(case when payment_method = 'app' then 1 else 0 end) 
/ count(*)) as pct_app_paid_orders from orders
where status = 'paid';
